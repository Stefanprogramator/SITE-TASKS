var quiz=document.getElementById("quiz");
var ques= document.getElementById("question");
var opt1=document.getElementById("option1");
var opt2=document.getElementById("option2");
var opt3=document.getElementById("option3");
var opt4=document.getElementById("option4");
var res=document.getElementById("result");
var nextbutton= document.getElementById("next");
var q=document.getElementById('quit');
var alerta = document.getElementById("alert1");
var mesaj = "SELECT AN OPTION!"
var mesaj2 = "ARE YOU SURE?"

var tques=questions.length;
var score=0;
var quesindex=0;

function myalert1(mesaj){

	alerta.style.display = "block";
	var msg = document.getElementById("mesaj");
	msg.innerHTML = mesaj;
	disableScreen();
}

function quit()
{         
	      quiz.style.display='none';
          result.style.display='';
          var f=score/tques;
          result.textContent="SCORE ="+f*100;
		  q.style.display="none";
		  recover2();
}
function give_ques(quesindex) 
{
	ques.textContent=quesindex+1+". "+questions[quesindex][0];
	opt1.textContent=questions[quesindex][1];
	opt2.textContent=questions[quesindex][2];
	opt3.textContent=questions[quesindex][3];
	opt4.textContent=questions[quesindex][4];
	 return;// body...
};
give_ques(0);
function nextques()
{
	var selected_ans= document.querySelector('input[type=radio]:checked');
	if(!selected_ans)
		{myalert1(mesaj);return;}

	if(selected_ans.value==questions[quesindex][5])
		{score=score+1;}
	selected_ans.checked=false;
	     quesindex++;
	     if(quesindex==tques-1)
	     	nextbutton.textContent="Finish";
	     var f=score/tques;
	     if(quesindex==tques)
	     {
	     q.style.display='none';
          quiz.style.display='none';
          result.style.display='';
          result.textContent="SCORED:"+(f*100).toFixed(2)+"%";
            return;
	     }
        give_ques(quesindex);

}

function disableScreen() {

    var div= document.createElement("div");
	div.className += "overlay";
	div.id += "overlay";
	document.body.appendChild(div);
	//document.body.style.position = "fixed";
}

function recover(){

	document.body.removeChild(document.getElementById("overlay"));
	alerta.style.display = "none";
	//document.body.style.position = "unset";	
}

function recover2(){

	var div = document.getElementById("alert2");
	var div1 = document.getElementById("fff");
	div1.removeChild(div);
	document.body.removeChild(document.getElementById("overlay"));
}


function myalert2(mesaj2){

	disableScreen();

	var body = document.getElementById("fff")

    var div1 = document.createElement("div");
	div1.className += "myalert2";
	div1.id += "alert2";
	
	var div2 = document.createElement("div");
    div2.id += "mesaj2";
    
    div2.innerHTML = mesaj2;

	var btn = document.createElement("button");
	var btn1 = document.createElement("button");
	btn.className += "buton";
	btn1.className += "buton";

	btn.innerHTML = "OK";
	btn1.innerHTML = "Cancel"

	btn.setAttribute( "onclick", "quit();" );
	btn1.setAttribute( "onclick", "recover2();" );

	div1.appendChild(div2);
	div1.appendChild(btn);
    div1.appendChild(btn1);
    
    body.appendChild(div1);


}


